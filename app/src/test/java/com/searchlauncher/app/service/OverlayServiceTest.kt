...
class OverlayServiceTest {
  ...
  fun `Left Edge - Swipe IN then OUT triggers show`() {
    ...
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_DOWN, 0f), isLeft = true)

    // 2. Swipe IN (Rightwards) past threshold (100f) -> X=150
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_MOVE, 150f), isLeft = true)

    // 3. Swipe OUT (Back to Left) below half threshold (50f) -> X=20
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_MOVE, 20f), isLeft = true)
  ...
  fun `Right Edge - Swipe IN then OUT triggers show`() {
    ...
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_DOWN, screenWidth), isLeft = false)
    ...
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_MOVE, 850f), isLeft = false)

    // 3. Swipe OUT (Back to Right).
    // Back near edge. X = 980. Delta is -20. deltaIn = 20 (< 50)
    gestureHandler.handleGesture(createEvent(MotionEvent.ACTION_MOVE, 980f), isLeft = false)
