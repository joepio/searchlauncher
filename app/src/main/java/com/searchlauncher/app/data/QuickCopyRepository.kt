...
class QuickCopyRepository(context: Context) {
    ...
    val items =
      try {
        ...
        List(jsonArray.length()) { i ->
          ...
          QuickCopyItem(alias = obj.getString("alias"), content = obj.getString("content"))
        }
      } catch (e: Exception) {
        emptyList()
      }
  ...
  suspend fun removeItem(alias: String) =
    withContext(Dispatchers.IO) {
      val currentItems = _items.value.toMutableList()
      currentItems.removeAll { it.alias.equals(alias, ignoreCase = true) }
      saveItems(currentItems)
  ...
  fun searchItems(query: String): List<QuickCopyItem> {
    ...
    return _items.value.filter {
      it.alias.lowercase().contains(lowerQuery) || it.content.lowercase().contains(lowerQuery)
    }
  }
}
