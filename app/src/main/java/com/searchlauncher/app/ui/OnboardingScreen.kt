  ...
  Column(
    ...
    Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
      ...
      Row(
        ...
        repeat(7) { index ->
          Box(
            modifier = Modifier.size(8.dp).then(if (index == currentStep) Modifier else Modifier)
          ) {
      ...
      if (currentStep > 0 && currentStep < 6) {
        TextButton(onClick = { currentStep++ }, modifier = Modifier.fillMaxWidth()) { Text("Skip") }
      }
    }
  }
}
    ...
    if (isGranted) {
      Card(
        ...
        Row(
          ...
          Text(text = "Permission Granted", color = MaterialTheme.colorScheme.onPrimaryContainer)
        }
      }
    } else {
